<footer class="bg-brand-emerald text-parchment mt-40 relative overflow-hidden">
    ...
    <div class="relative py-24 lg:py-32">
        <div class="container-wide grid md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16 relative z-10">
            ...
            <div class="lg:col-span-1">
                <h3 class="text-3xl font-serif italic mb-8">{{ App\Models\Setting::getValue('site_title', 'Royal Dine') }}</h3>
                ...
                <div class="flex gap-4">
                    <a href="{{ App\Models\Setting::getValue('social_instagram_url', '#') }}" target="_blank" class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-brand-gold hover:bg-brand-gold hover:text-brand-emerald transition-all duration-300">
                    ...
                    <a href="{{ App\Models\Setting::getValue('social_facebook_url', '#') }}" target="_blank" class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-brand-gold hover:bg-brand-gold hover:text-brand-emerald transition-all duration-300">
            ...
            <div>
                ...
                <ul class="flex flex-col gap-4 text-sm font-medium text-parchment/80">
                    <li class="flex justify-between items-center border-b border-white/10 pb-3">
                        <span>Mon - Thu</span>
                        <span class="text-brand-gold">{{ App\Models\Setting::getValue('footer_hours_mon_thu', '12pm - 11pm') }}</span>
                    </li>
                    <li class="flex justify-between items-center border-b border-white/10 pb-3">
                        <span>Fri - Sun</span>
                        <span class="text-brand-gold">{{ App\Models\Setting::getValue('footer_hours_fri_sun', '2pm - 12am') }}</span>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="border-t border-white/5 py-8">
        <div class="container-wide flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] uppercase font-bold tracking-widest text-parchment/40">
            <p>&copy; {{ date('Y') }} {{ App\Models\Setting::getValue('site_title', 'Royal Dine') }}. All rights reserved.</p>
